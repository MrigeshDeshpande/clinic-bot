...
export default function ProvisionalDiagnosisCard({ diagnosisOptions = [], selectedDiagnoses = [], diagnosisNotes = '', onToggleDiagnosis, onNotesChange }) {
  ...
  return (
    <div>
      <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-2">Provisional Diagnosis</h3>
      <div className="space-y-2">
        ...
        <textarea value={diagnosisNotes} onChange={e => onNotesChange(e.target.value)}
          rows={2} className="w-full px-2.5 py-1.5 text-xs bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-200 dark:focus:ring-blue-800 transition-all resize-none placeholder-gray-400"
