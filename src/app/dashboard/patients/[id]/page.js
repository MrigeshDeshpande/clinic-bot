      ...
      <div className="max-w-6xl mx-auto p-4 md:p-6 lg:p-8 space-y-6">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-3xl border border-gray-100 dark:border-gray-800 p-4 md:p-8 shadow-sm transition-colors duration-200">
                ...
                <div className="min-w-0">
                  ...
                  {family.length > 0 && (
                    <div className="mt-3 flex flex-wrap items-center gap-2">
                      <span className="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider mr-1">Family Members:</span>
                      {family.map(member => (
                        <div key={member.id} className="inline-flex items-center gap-1.5 pl-2.5 pr-1.5 py-1 bg-gray-50 dark:bg-gray-800 rounded-xl border border-gray-200/80 dark:border-gray-700 text-xs font-medium text-gray-700 dark:text-gray-300">
                          <span className="cursor-pointer hover:underline text-gray-900 dark:text-gray-100 font-bold" onClick={() => router.push(`/dashboard/patients/${member.id}`)}>
                            {member.name}
                          </span>
                          <span className="text-[10px] text-gray-400 font-normal capitalize">({member.relationship_type})</span>
                          <button
                            type="button"
                            onClick={() => handleUnlinkPatient(member.relationship_id, member.name)}
                            disabled={unlinking === member.relationship_id}
                            className="p-0.5 text-gray-400 hover:text-red-500 dark:hover:text-red-400 rounded-md hover:bg-gray-100/50 dark:hover:bg-gray-700 transition-colors"
                          >
                            <X className="w-3 h-3" />
                          </button>
                        </div>
          ...
          {completedVisits.length > 0 && (
            <>
              <button onClick={() => visitsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' })} className="w-full mt-4 sm:mt-6 flex flex-wrap items-center gap-1.5 text-xs text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 transition-all text-left">
