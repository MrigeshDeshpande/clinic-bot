...
export async function generatePrescription({ patient, visit, appointment }) {
  ...
  if (toothDiagnoses.length > 0) {
    ...
    const colX = [LM, LM + 38, LM + 70];
    const colW = [34, 32, RW - 70];
    const tableW = RW;
    const rh = 16;
    ...
    for (let i = 0; i < toothDiagnoses.length; i++) {
      ...
      // Vertical separators
      doc.moveTo(colX[1], y).lineTo(colX[1], y + rowH).strokeColor('#e5e7eb').lineWidth(0.5).stroke();
      doc.moveTo(colX[2], y).lineTo(colX[2], y + rowH).strokeColor('#e5e7eb').lineWidth(0.5).stroke();
