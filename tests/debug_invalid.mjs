...
const waId = 'debug_invalid_' + Date.now();
...
for (let i = 0; i < messages.length; i++) {
  ...
  try {
    ...
    if (result && result.steps) {
      ...
      console.log(`  Intent: ${s.intent} (source: ${s.intentSource})`);
      console.log(`  State: ${s.state} (prev: ${s.previousState}${s.nextState !== null ? `, nextState: ${s.nextState}` : ''})`);
